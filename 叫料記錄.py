...
# 設定檔
CSV_FILE = Path("/Users/herman/file/叫料記錄.csv")
...
class App:
    ...
    def setup_styles(self):
        ...
        # 按鈕樣式
        self.style.configure("Primary.TButton", font=("Microsoft JhengHei", 11), padding=10)
        self.style.configure("TButton", font=("Microsoft JhengHei", 10), padding=8)
        
        # 標籤樣式
        self.style.configure("TLabel", font=("Microsoft JhengHei", 11), background="#f5f5f5")
        self.style.configure("Header.TLabel", font=("Microsoft JhengHei", 14, "bold"), background="#f5f5f5")
        
        # 表格樣式
        self.style.configure("Treeview", font=("Microsoft JhengHei", 10), rowheight=28)
        self.style.configure("Treeview.Heading", font=("Microsoft JhengHei", 11, "bold"))
    ...
    def create_widgets(self):
        ...
        buttons = [
            ("➕ 新增", self.add_record),
            ("✏️ 修改", self.edit_record),
            ("🗑️ 刪除", self.delete_record),
        ...
        for text, cmd in buttons:
            btn = tk.Button(
                btn_frame, text=text, font=("Microsoft JhengHei", 11),
                bg="#3498db", fg="black", relief="flat", padx=15, pady=8,
        ...
        tk.Label(search_frame, text="🔍 搜尋：", font=("Microsoft JhengHei", 11), bg="#f5f5f5", fg="black").pack(side="left")
        
        self.search_var = tk.StringVar()
        self.search_var.trace("w", self.on_search)
        search_entry = tk.Entry(search_frame, textvariable=self.search_var, font=("Microsoft JhengHei", 11), width=30)
        search_entry.pack(side="left", padx=5)
        
        tk.Button(search_frame, text="清除", font=("Microsoft JhengHei", 10), 
        ...
        tk.Label(filter_frame, text="🔍 篩選狀態：", font=("Microsoft JhengHei", 11), bg="#f5f5f5", fg="black").pack(side="left")
        ...
        for status in status_options:
            rb = tk.Radiobutton(
                ...
                font=("Microsoft JhengHei", 10), bg="#f5f5f5", fg="black", activebackground="#f5f5f5",
        ...
        self.stats_label = tk.Label(
            self.root, text="", font=("Microsoft JhengHei", 10),
            bg="#f5f5f5", fg="black"
        )
        ...
        self.status_bar = tk.Label(
            ...
            bg="#ecf0f1", fg="black", anchor="w", padx=10
        )
    ...
    def batch_edit_status(self):
        ...
        tk.Label(batch_win, text="選擇要修改的狀態：", font=("Microsoft JhengHei", 12)).pack(pady=20)
        
        # 新狀態選單
        new_status_var = tk.StringVar()
        status_combo = ttk.Combobox(batch_win, textvariable=new_status_var, values=CONTACT_STATUSES, font=("Microsoft JhengHei", 11), width=20)
        status_combo.pack(pady=10)
        
        # 說明
        tk.Label(batch_win, text="將選取的所有記錄狀態改為：", font=("Microsoft JhengHei", 10), fg="gray").pack()
        ...
        def do_batch_edit():
            ...
            selection = self.tree.selection()
            if not selection:
                messagebox.showwarning("請選擇", "請先選取要修改的記錄（可多選）")
                return
        ...
        tk.Button(batch_win, text="確認修改", font=("Microsoft JhengHei", 12), 
    ...
    def open_form(self, record=None, idx=None):
        ...
        # 滾動區域
        canvas = tk.Canvas(form, bg="#f5f5f5", highlightthickness=0)
        ...
        # 確保 scrollable_frame 寬度與 canvas 一致
        ...
        for field, label, default in all_fields:
            ...
            lbl = tk.Label(
                ...
                font=("Microsoft JhengHei", 11), bg="#f5f5f5", fg="black", width=12, anchor="e"
            )
            lbl.pack(side="left")
            ...
            if field == "工程師":
                ...
                combo = ttk.Combobox(frame, textvariable=var, values=ENGINEERS, font=("Microsoft JhengHei", 11), width=25)
                combo.pack(side="left", fill="x", expand=True)
                entries[field] = var
                
            elif field == "事項":
                var = tk.StringVar(value=value)
                combo = ttk.Combobox(frame, textvariable=var, values=ITEM_TYPES, font=("Microsoft JhengHei", 11), width=25)
                combo.pack(side="left", fill="x", expand=True)
                entries[field] = var
                
            elif field == "是否收取訂金":
                var = tk.StringVar(value=value)
                combo = ttk.Combobox(frame, textvariable=var, values=DEPOSIT_METHODS, font=("Microsoft JhengHei", 11), width=25)
                combo.pack(side="left", fill="x", expand=True)
                entries[field] = var
                
            elif field == "聯繫狀態":
                var = tk.StringVar(value=value)
                combo = ttk.Combobox(frame, textvariable=var, values=CONTACT_STATUSES, font=("Microsoft JhengHei", 11), width=25)
                combo.pack(side="left", fill="x", expand=True)
                entries[field] = var
                
            elif field in ["料件備註", "其餘備註"]:
                text = tk.Text(frame, font=("Microsoft JhengHei", 11), width=28, height=3, wrap="word")
                ...
                var = tk.StringVar(value=value)
                entry = tk.Entry(frame, textvariable=var, font=("Microsoft JhengHei", 11), width=28)
        ...
        tk.Button(
            btn_frame, text="💾 儲存", font=("Microsoft JhengHei", 12),
            bg="#27ae60", fg="black", relief="flat", padx=20, pady=8,
            cursor="hand2", command=save
        ).pack(side="right", padx=5)
        
        tk.Button(
            btn_frame, text="✖ 取消", font=("Microsoft JhengHei", 12),
            bg="#95a5a6", fg="black", relief="flat", padx=20, pady=8,
